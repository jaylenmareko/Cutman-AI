...
import { Plus, Video, AlertTriangle, FileText, Loader2 } from "lucide-react";
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
            <div>
              <h1 className="text-4xl sm:text-5xl font-display uppercase tracking-wider">SCOUTING REPORTS</h1>
              <p className="text-muted-foreground mt-2 text-lg">Manage and review your fighter analyses.</p>
            </div>
            <Link href="/new">
          ...
          {isLoading ? (
                    ...
                    <Card className="h-full cursor-pointer hover:border-primary transition-colors duration-300 group overflow-hidden relative">
                      ...
                      <CardContent className="p-6 flex flex-col h-full">
                        ...
                        <div className="mt-auto">
                          <h3 className="text-2xl font-display uppercase tracking-wide truncate mb-1" title={report.fighterName || "Unknown Fighter"}>
                            {report.fighterName || "UNKNOWN FIGHTER"}
                          </h3>
            ...
            <div className="text-center py-24 bg-card rounded-lg border border-dashed border-border">
              ...
              <p className="text-muted-foreground text-lg mb-8 max-w-md mx-auto">
                Upload your first fight video to generate an AI-powered scouting report.
              </p>
              <Link href="/new">
...
function StatusBadge({ status }: { status: string }) {
  switch (status) {
    ...
    case "error":
      return (
        <Badge variant="destructive" className="font-bold tracking-wide">
    ...
    default:
      return (
        <Badge variant="outline" className="bg-yellow-500/10 text-yellow-500 border-yellow-500/20 hover:bg-yellow-500/20 font-bold tracking-wide animate-pulse">
          PROCESSING
        </Badge>
      );
  }
